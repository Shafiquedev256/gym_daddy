...
export const Gender = ()=>{
  const {updateData} = useUserInfo();
  const dataEl = useUserInfo().dataEl;
  return(
    <> 
    {dataEl&&dataEl().gender}
...
<img src={male} className="w-[100%] h-[240px]" onClick={() => updateData && updateData({gender: "Male"})} />
...
<img src={female} className="w-[100%] " onClick={() => updateData && updateData({gender: "Female"})} />
